...
if 0:
    ...
    for p in range(6, 16, 1):
        ...
        h_sw0 = h_pf*2.1 #0.3 
        h_rw0 = h_pf*2.1 #0.3    
        n_iters_s, n_times_s = 100, 2.5
        n_iters_r, n_times_r = 70, 3
...
if 1:
    ...
    ax2.scatter(lst_pole_pairs, [l*1e-3 for l in lst_HTS_length], color = "red", label="HTS length")
...
if 1:
    ...
    ax1.set_xlabel("pole pairs")
    ax1.scatter(lst_pole_pairs, [w*1e-3 for w in lst_weight_rotor_yoke], color = "red", label="weight rotor yoke")    
    ax1.plot(lst_pole_pairs, [w*1e-3 for w in lst_weight_rotor_yoke], color = "red")
    ax1.set_ylabel("Generator Weight / t")
    
    ax1.scatter(lst_pole_pairs, [w*1e-3 for w in lst_weight_stator_yoke], color = "green", label="weight stator yoke")    
    ax1.plot(lst_pole_pairs, [w*1e-3 for w in lst_weight_stator_yoke], color = "green")
    ...
    if export_png: plt.savefig(fname = preambel + "weight_rotor_stator_yoke.png")
...
if 1:
    ...
    ax1.scatter(lst_pole_pairs, [w*1e-3 for w in lst_weight_rotor_wndg], color = "red", marker="x", label="weight rotor wndg")    
    ax1.plot(lst_pole_pairs, [w*1e-3 for w in lst_weight_rotor_wndg], color = "red")
    
    ax1.scatter(lst_pole_pairs, [w*1e-3 for w in lst_weight_rotor_kryo], color = "green", marker="x", label="weight rotor kryo")    
    ...
    if export_png: plt.savefig(fname = preambel + "weight_rotor_stator_wndg_kryo.png")
    
    if export_tkz:
        tkz.clean_figure()
        tkz.save(preambel + "weight_rotor_stator_wndg_kryo.tex")
