...
num = U_i + jXdIs
den = np.sqrt(2) * 2 * generator.f * generator.ks_d * generator.ks_p * generator.l_e * tau_p * B_delta_hat

new_N_s = int(np.round(num / den,0))
new_N_c_div_a = new_N_s / (2 * generator.p * generator.q)

new_J_s_amplitude = np.sqrt(2) * I_s * new_N_c_div_a / generator.coil.A_sc

print(f"{new_N_s = }")
print(f"{new_J_s_amplitude = }")
